	...
	public static function send( $settings = array(), $file, $send_id = '', $delete_after = false ) {
		...
		if ( $settings['_chunk_upload_id'] != '' ) {
			...
			if ( true === feof( $f ) ) {
				...
				try {
					$result = self::$_dbxClient->chunkedUploadFinish( $settings['_chunk_upload_id'], rtrim( $settings['directory'], '/\\' ) . '/' . basename( $file ), dbx\WriteMode::add(), filesize( $file ) );
				} catch ( \Exception $e ) {
			...
			if ( ( $settings['max_chunk_size'] >= 5 ) && ( ( $file_size / 1024 / 1024 ) > $settings['max_chunk_size'] ) ) { // chunked send.
				...
				pb_backupbuddy::status( 'details', 'Send Directory: ' . $settings['directory'] );
				$send_time = -(microtime( true ));
				try {
					$result = self::$_dbxClient->uploadFile( rtrim( $settings['directory'], '/\\' ) . '/' . basename( $file ), dbx\WriteMode::add(), $f, $file_size );
				} catch ( \Exception $e ) {
					pb_backupbuddy::status( 'error', 'Dropbox Error: ' . $e->getMessage() );
					return false;
				}
