<?php
/**
 * Stash v2 Destination main class.
 *
 * @package BackupBuddy
 */
 ...
 */
class pb_backupbuddy_destination_stash2 {
	...
	public static function listFiles( $settings, $mode = 'default' ) {
		...
		foreach ( (array) $backups as $file ) {
			...
			if ( 'default' === $mode ) {
				if ( 'live' === $settings['type'] ) {
					$copy_link = pb_backupbuddy::ajax_url( 'live_stash_files' ) . '&cpy=' . rawurlencode( $backup );
				} else {
	...
	public static function deleteFiles( $settings, $files = array() ) {
		...
		$stash_action = 'manage';
		$manage_data  = self::stashAPI( $settings, $stash_action );
