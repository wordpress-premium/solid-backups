...
// 				}
		...
		protected function create_generic( $zip, $dir, $excludes, $tempdir, $listmaker ) {
			...
			// Always grow because we always have zip achive with at least one file present
			$command .= ' -g';
			...
			// Always create the zip archive in the temporary location and if is built successfully
			// with no errors or warnings (or with warnings but configured to ignore these) then we
			// move itto the actual local archive storaeg directory. Note that we used to build in
			// the final location if warnings were being ignored but this really had little benefit
			// so better to make all methods behave in teh same way.
			...
			if ( function_exists( 'exec' ) ) {
				...
				switch ( (int) $exit_code ) {
					case 0:
						...
						pb_backupbuddy::status( 'details', sprintf( __( 'exec (unzip) listed file contents (%1$s) - generic', 'it-l10n-backupbuddy' ), $zip_file ) );
			...
			if ( function_exists( 'exec' ) ) {
				...
				switch ( (int) $exit_code ) {
					case 0:
						...
						pb_backupbuddy::status( 'details', sprintf( __( 'exec (unzip) retrieved file contents for (%1$s)', 'it-l10n-backupbuddy' ), $zip_file . '/' . $file_path ) );
					...
					default:
						// For now let's just print the error code and drop through
						$error_string = $exit_code;
						pb_backupbuddy::status( 'details', sprintf( __('exec (unzip) failed to open/process file to list contents (%1$s) - Error Info: %2$s.','it-l10n-backupbuddy' ), $zip_file . '/' . $file_path, $error_string ) );
