...
//
		...
		public function create_empty_zip( $zip, $comment, $tempdir ) {
			...
			pb_backupbuddy::status( 'details', sprintf( __( 'Zip process reported: Initializing zip archive file: %1$s', 'it-l10n-backupbuddy' ), $zip_file_name ) );
			...
			try {
				...
				pb_backupbuddy::status( 'details', sprintf( __( 'Zip process reported: Initialized zip archive file', 'it-l10n-backupbuddy' ) ) );
				$result = true;

			} catch ( Exception $e ) {

				$error_string = $e->getMessage();
				pb_backupbuddy::status( 'details', sprintf( __('Zip process reported: Failure to initialize zip archive file - error reported: %1$s','it-l10n-backupbuddy' ), $error_string ) );
			}
		...
		public function add_virtual_file_to_zip( $zip_file, $virtual_file_descriptor, $temporary_zip_directory = '' ) {
			...
			if ( empty( $zip_methods ) ) {
				...
				pb_backupbuddy::status( 'details', __('Failed to add virtual file - no available methods.','it-l10n-backupbuddy' ) );
			...
			pb_backupbuddy::status( 'details', __('Failed to add virtual file with any nominated method.','it-l10n-backupbuddy' ) );
		...
		public function get_file_list( $zip_file ) {
			...
			if ( empty( $zip_methods ) ) {
				// Hmm, we don't seem to have any available methods, oops, best go no further.
				pb_backupbuddy::status( 'details', __( 'Failed to get zip file list - no available methods.', 'it-l10n-backupbuddy' ) );
				return false;
			}
			...
			foreach ( $zip_methods as $method_tag ) {
				...
				if ( true === $this->_zip_methods_details[ $method_tag ]['attr']['is_lister'] ) {
		...
		public function get_file_contents( $zip_file, $file_path ) {
			...
			if ( empty( $zip_methods ) ) {
				...
				// Hmm, we don't seem to have any available methods, oops, best go no further
				pb_backupbuddy::status( 'details', __('Failed to get zip file contents - no available methods.', 'it-l10n-backupbuddy' ) );
