<?php
/**
 * Solid Backups MySQLBuddy class
 *
...
/**
 * MySQL Buddy Main Class
 */
class pb_backupbuddy_mysqlbuddy {

	/**
	 * If command line is longer than this then we will try to determine max command line length.
	 *
	 * @var int
	 */
	const COMMAND_LINE_LENGTH_CHECK_THRESHOLD = 250;
	 ...
	 * @var int
	 */
	const TIME_WIGGLE_ROOM = 4;

	/**
	 * Number of rows/queries to heartbeat after, logging number of rows handled.
	 *
	 * @var int
	 */
	const HEARTBEAT_COUNT_LIMIT = 2000;

	/**
	 * Internal version number for this library.
	 *
	 * @var string
	 */
	...
	/**
	 * If using sockets, points to socket file. Left blank if sockets not in use.
	 *
	 * @see __construct()
	 *
	 * @var string
	 */
	private $_database_socket = '';
	 ...
	 * @var string
	 */
	private $_database_pass = '';

	/**
	 * Database table prefix to backup when in prefix mode.
	 *
	 * @see __construct()
	 *
	 * @var string
	 */
	private $_database_prefix = '';

	/**
	 * Database port to FORCE. Default blank. If set then this port will be specified in connection. Leave blank to use default for server.
	 *
	 * @see __construct()
	 *
	 * @var string
	...
	/**
	 * Determines base tables to include in backup. Ex: none, all, or by prefix.
	 *
	 * @see __construct()
	 *
	 * @var string
	 */
	private $_base_dump_mode = '';

	/**
	 * Additional tables to backup in addition to those determined by base mode.
	 *
	 * @var array
	 */
	private $_additional_includes = array();

	/**
	 * Tables to exclude from ( $_additional_includes + those determined by base mode ).
	 *
	 * @var array
	 */
	private $_additional_excludes = array();

	/**
	 * Available mechanisms for dumping in order of preference.
	 *
	 * @var array
	 */
	...
	/**
	 * Populated by _calculate_mysql_directory().
	 *
	 * @var array
	 */
	private $_mysql_directories = array();

	/**
	 * If mysql tells us where its installed we prepend to this. Beginning and trailing slashes.
	 *
	 * @var array
	 */
	private $_default_mysql_directories = array(
		'/usr/bin/',
		'/usr/bin/mysql/',
		'/usr/local/bin/',
	);
	...
	/**
	 * Tested working mysql directory to use for actual dump.
	 *
	 * @var string
	 */
	private $_mysql_directory = '';

	/**
	 * Instance of commandbuddy class.
	 *
	 * @var object
	 */
	...
	/**
	 * Version of the mysql server that dumped this SQL. Handle workarounds of SQL syntax changes.
	 *
	 * @var string
	 */
	private $_incoming_sql_version = '';

	/**
	 * Version of the mysql server running locally, calculated on construct.
	 *
	 * @var string
	 */
	private $_current_sql_version = '';

	/**
	 * Whether or not to migrate SQL syntax for incoming mysql < v5.1 when importing to mysql 5.1+.
	 *
	 * @var bool
	 */
	private $_hotfix_7001 = false;
	...
	/**
	 * Number of 9010 Errors encountered.
	 *
	 * @var int
	 */
	...
	/**
	 * After this point an error will be shown indicating subsequent errors will be hidden. All 9010 log to: ABSPATH . 'importbuddy/mysql_9010_log-' . pb_backupbuddy::$options['log_serial'] . '.txt'
	 *
	 * @var int
	 */
	private $_max_9010s_allowed = 9;

	/**
	 * When enabled, Solid Backups will dump individual tables to their own database file (eg wp_options.sql, wp_posts.sql, etc) when possible based on other criteria such as the dump method and whether breaking out big tables is enabled.
	 *
	 * @var bool
	 */
	private $_force_single_db_file = false;

	/**
	 * Current SQL file operated on. Used for logging.
	 *
	 * @var string
	 */
	private $_current_sql_file = '';

	/**
	 * Default rows per select is determined by default private var above if none passed. Overriden by pb_backupbuddy::$options['phpmysqldump_maxrows'] if not blank and is numeric and overridden by passed parameter if defined.
	 *
	 ...
	 * @param array  $force_methods        Optional. Possible methods: commandline, php - Override automatic method detection. Skips test and runs first method first. Falls back to other methods if any failure.
	 * @param int    $max_execution        Optional. Maximum execution time to run for before stopping to allow for continuing the next import picking up where we left off. If blank we try and deduce it. If -1 then we do NOT use chunking. Useful for classic mode.
	 * @param int    $max_rows_per_select  Optional. For PHP-based mysql dump, max number of rows per select to grab.
	 */
	public function __construct( $database_host, $database_name, $database_user, $database_pass, $database_prefix, $force_methods = array(), $max_execution = '', $max_rows_per_select = '' ) {
		...
		$this->_current_sql_version = PB_Backupbuddy_DB_Helpers::get_db_version();
		if ( PB_Backupbuddy_DB_Helpers::is_maria_db() ) {
			pb_backupbuddy::status( 'details', 'This server\'s mariadb version: `' . $this->_current_sql_version . '`.' );
		} else {
	 ...
	 *
	 * @param string $version  Incoming SQL version number.
	 */
	...
	/**
	 * When enabled by passing true, Solid Backups will dump individual tables to their own database file (eg wp_options.sql, wp_posts.sql, etc) when possible based on other criteria such as the dump method and whether breaking out big tables is enabled.
	 *
	...
	/**
	 * Returns available dump methods
	 *
	...
	/**
	 * Dumps SQL based on chosen method.
	 *
	 * @see _mysqldump()
	 * @see _phpdump()
	 *
	 ...
	 */
	public function dump( $output_directory, $tables = array(), $rows_start = 0 ) {
		...
		pb_backupbuddy::status( 'details', 'mysqlbuddy: Output directory: `{' . htmlspecialchars( $output_directory ) . '}`.' );
		if ( count( $tables ) == 1 ) {
		...
		foreach ( $this->_methods as $method ) {
			...
			if ( method_exists( $this, "_dump_{$method}" ) ) {
				...
				if ( true === $result ) { // Dump completed succesfully with this method.
					pb_backupbuddy::status( 'details', 'mysqlbuddy: Dump method `' . $method . '` completed successfully.' );
					$return = true;
					break;
				} elseif ( false === $result ) { // Dump failed this method. Will try compatibility fallback to next mode if able.
	 ...
	 * @return bool  If successful or not.
	 */
	private function _dump_commandline( $output_directory, $tables, $rows_start = 0 ) {
		...
		pb_backupbuddy::status( 'sqlFile', basename( htmlspecialchars( $output_file ) ) ); // Tells status checker which file to request size data for when polling server.
		...
		*/
				...
				if ( is_array( $exec_output ) && is_numeric( $exec_output[0] ) ) {
					...
					if ( $command_length > ( $exec_output[0] - 100 ) ) { // Check if we exceed maximum length. Subtract 100 to make room for path definition.
						if ( '1' == pb_backupbuddy::$options['ignore_command_length_check'] ) {
							...
							if ( $exec_output[0] < 0 ) {
								pb_backupbuddy::status( 'warning', 'mysqlbuddy: This system reported a negative number for the maximum command line length. This means that Solid Backups cannot safely detect this system\'s limit.  This check can be overridden by enabling the "Skip command line length check" advanced option on the Settings page in Solid Backups as a workaround. get_conf ARG_MAX likely failed.' );
							}
		...
		// Check the result of the exit code.
		if ( '0' == $exec_exit_code ) {
			...
			if ( file_exists( $output_file ) ) { // SQL file found. SUCCESS!
				...
				pb_backupbuddy::status( 'details', 'Final SQL database dump file size of `' . basename( htmlspecialchars( $output_file ) ) . '`: ' . $sql_filesize . '.' );
				pb_backupbuddy::status( 'finishTableDump', '' );

				return true;
			} else { // SQL file MISSING. FAILURE!
				pb_backupbuddy::status( 'error', 'mysqlbuddy: Though command reported success database dump SQL file is missing: `' . htmlspecialchars( $output_file ) . '`.' );
				return false;
			}
		} else {
	 ...
	 * @param string $output_directory     Directory to output to. May also be used as a temporary file location. Trailing slash required. dump() auto-adds trailing slash before passing.
	 * @param array  $tables               Array of tables to dump.
	 * @param int    $resume_starting_row  Starting row, if any. Only works with PHP mode. Used with chunking.
	 *
	 * @return bool  If successful or not.
	 */
	private function _dump_php( $output_directory, $tables, $resume_starting_row = 0 ) {
		...
		if ( ! is_object( $wpdb ) ) {
			...
			error_log( 'WordPress database object $wpdb did not exist. This should not happen. Solid Backups Error #8945587973.' );
			return false;
		}
		...
		foreach ( $tables as $table_key => $table ) {
			...
			pb_backupbuddy::status( 'details', 'SQL dump file `' . htmlspecialchars( $output_file ) . '`.' );
			if ( '-1' == $this->_maxExecutionTime ) {
			...
			if ( false === ( $file_handle = fopen( $output_file, 'a' ) ) ) {
				pb_backupbuddy::status( 'error', 'Error #9018: Database file is not creatable/writable. Check your permissions for file `' . htmlspecialchars( $output_file ) . '` in directory `' . htmlspecialchars( $output_directory ) . '`.' );
				return false;
			}
			...
			while ( true === $rows_remain ) {
				...
				pb_backupbuddy::status( 'details', 'Got `' . $table_count . '` rows from `' . $table . '` of `' . $max_rows_per_select . '` max.' );
				if ( 0 == $table_count || $table_count < $max_rows_per_select ) {
					$rows_remain = false;
				}
				...
				foreach ( $table_query as $fetch_row ) {
					...
					if ( false === $write_return || 0 == $write_return ) {
						pb_backupbuddy::status( 'error', 'Error #843948: Unable to write to SQL file. Return error/bytes written: `' . $write_return . '`.' );
						@fclose( $file_handle );
						return false;
					}
					$insert_sql = '';
				...
				if ( false === $rows_remain ) {
					...
					if ( ( microtime( true ) - $last_file_size_output ) > $output_file_size_every_max ) { // It's been long enough to get the current file size of SQL file.
						// Display final SQL file size.
						$sql_filesize = pb_backupbuddy::$format->file_size( filesize( $output_file ) );
						pb_backupbuddy::status( 'details', 'Current database dump file `' . basename( htmlspecialchars( $output_file ) ) . '` size: ' . $sql_filesize . '.' );
						$last_file_size_output = microtime( true );
					}
			...
			unset( $remaining_tables[ $table_key ] );
			...
			if ( isset( $output_file ) ) {
				...
				pb_backupbuddy::status( 'details', 'Database SQL dump file (' . basename( htmlspecialchars( $output_file ) ) . ') size: ' . pb_backupbuddy::$format->file_size( $stats['size'] ) );
	 ...
	 * @param bool   $ignore_existing  NOTE: Basically ignores mysql errors these days. Past purpose: Whether or not to allow import if tables exist already. Default: false.
	 *
	 * @return mixed  true on success (boolean), false on failure (boolean), integer (int) on needing a resumse (integer is resume number for next page loads $query_start).
	 */
	...
	/**
	 * Check if string begins with specific characters
	 *
	 * @param string $string  String to search.
	 * @param string $search  Characters to check if search string begins with.
	 *
	 * @return bool  Whether or not it begins with the search string.
	 */
	 ...
	 * Using codebase from Solid Backups / importbuddy 2.x.
	 *
	 * @see import()
	 * @since 2.0.0
	 *
	 * @param string $sql_file         Full absolute path to .sql file to import from.
	 * @param string $old_prefix       Old database prefix. New prefix provided in constructor.
	 * @param string $resume_point     Point at which to resume.
	 * @param bool   $ignore_existing  NOTE: Basically ignores mysql errors these days. Past purpose: Whether or not to allow import if tables exist already. Default: false.
	 *
	 * @return bool  True on success (and completion), incomplete/chunking required= array( file pointer[for resuming], queries so far[informative only] ), false on failure.
	 */
	public function _import_php( $sql_file, $old_prefix, $resume_point = '', $ignore_existing = false ) {
		$this->time_start = microtime( true );
		...
		if ( '' != $resume_point && $resume_point > 0 ) {
			if ( 0 !== fseek( $fs, $resume_point ) ) { // Returns 0 on success.
				pb_backupbuddy::status( 'error', 'Error #9483453: Failed to seek SQL file to resume point `' . $resume_point . '` via fseek().' );
				return false;
			}
		}
		...
		if ( false === $results ) {
			pb_backupbuddy::status( 'warning', 'Warning #8493844: ' . @mysqli_error( $wpdb->dbh ) );
		}
		...
		while ( false !== ( $query = fgets( $fs ) ) ) { // Grab line by line as long as fgets doesn't return FALSE.
			...
			if ( false === strpos( $query, ';' ) ) { // Don't have the full query yet. Need to grab more lines.
				$prev_query_data .= $query;
				continue; // Go grab more.
			}

			// Execute the SQL line.
			$result = $this->_import_sql_dump_line( trim( $prev_query_data . $query ), $old_prefix, $ignore_existing ); // Added trim() as of BB v5 to rip off sometimes-leading newline char.
			if ( false === $result ) {
				// Got FALSE as response... Skipped query so continue to the next.
				$failed_queries++;
				continue;
			}
			$prev_query_data = ''; // Clear out any previous query data that was not used due to not having the full statement.
			$query_count++;

			// Show a heartbeat to keep user up to date [and entertained ;)].
			if ( 0 === ( $query_count % self::HEARTBEAT_COUNT_LIMIT ) ) { // Display every X queries.
			...
			if ( '-1' != $this->_maxExecutionTime ) { // -1 disabled chunking.
				if ( ( ( microtime( true ) - pb_backupbuddy::$start_time ) + self::TIME_WIGGLE_ROOM ) >= $this->_maxExecutionTime ) { // was $this->time_start but did not take into account time used up before db stuff.
					...
					if ( false === $results ) {
						pb_backupbuddy::status( 'warning', 'Warning #39379792434: ' . @mysqli_error( $wpdb->dbh ) );
					}
		...
		$results = $wpdb->query( 'SET FOREIGN_KEY_CHECKS=1;' );
		if ( false === $results ) {
			pb_backupbuddy::status( 'warning', 'Warning #858558549: ' . @mysqli_error( $wpdb->dbh ) );
		}
	 ...
	 * @param string $old_prefix       New prefix was passed in constructor.
	 * @param bool   $ignore_existing  If true suppress mysql errors.
	 *
	 * @return bool True=success, False=failed.
	 */
	public function _import_sql_dump_line( $query, $old_prefix, $ignore_existing = false ) {
		...
		if ( $old_prefix !== $new_prefix ) {
			$query = preg_replace( "/^($query_operators)(\s+`?)$old_prefix/i", "\${1}\${2}$new_prefix", $query ); // Added on 4-29-11.
		}
		...
		if ( false === $results ) {
			if ( true !== $ignore_existing ) {
				...
				if ( $this->_9010s_encountered > $this->_max_9010s_allowed ) { // Too many, don't show more.
					...
					pb_backupbuddy::status( 'error', 'Error #9010: Unable to import SQL query. Error: `' . htmlspecialchars( $mysql_error ) . '` Errno: `' . $mysql_errno . '`.' );
					if ( false !== stristr( $mysql_error, 'server has gone away' ) ) { // if string denotes that mysql server has gone away bail since it will likely just flood user with errors...
						...
						pb_backupbuddy::status( 'details', 'Last query attempted: `' . htmlspecialchars( $query ) . '`.' );
