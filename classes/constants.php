...
class backupbuddy_constants {
	...
	const PHP_RUNTIME_TEST_MAX_TIME = 250; // Maximum number of seconds (loops with 1sec sleeps) to run PHP runtime test for. For huge runtimes prevents from running for full duration.
	...
	// Cron
	const CRON_HOOK = 'backupbuddy_cron'; // Cron hook name.
	const CRON_GROUP = 'solid-backups'; // Action Scheduler Group Name.
	const DEFAULT_CRON_PRIORITY = 5; // Default cron priority for registering via add_action().
	const CRON_SINGLE_PASS_RESCHEDULE_LIMIT = 1000; // Safety net to prevent infinite rescheduling. Should never happen, but just in case.
