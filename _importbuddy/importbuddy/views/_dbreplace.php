<?php
if ( ! is_admin() ) { // Not in WordPress (or not logged in). Check if in the Importer.
	if ( ! defined( 'PB_IMPORTBUDDY' ) || ( true !== PB_IMPORTBUDDY ) ) { // Not in the Importer.
		die( '<html></html>' );
	}
	// In the Importer so check authentication.
...
echo '<br><b>Note:</b> The Importer automatically handles migrating & replacing your site URLs and file paths during restore/migration; this tool is not needed for normal backup / restore operations.';
...
echo '<p><span class="pb_label pb_label-warning">Caution</span> This is an advanced feature. Use with care; improper use may result in data loss.</p>';
echo '<br>';
...
if ( pb_backupbuddy::_GET( 'database_replace' ) == '1' ) {
	...
	echo '<div id="pb_importbuddy_working"><img src="' . pb_backupbuddy::plugin_url() . '/assets/dist/images/loading_large.gif" title="Working... Please wait as this may take a moment..."></div>';
	echo '<script>jQuery("#pb_backupbuddy_status_wrap").show();</script>';
	pb_backupbuddy::flush();
	//echo '<div id="pb_backupbuddy_replace_working"><img src="' . pb_backupbuddy::plugin_url() . '/assets/dist/images/loading_large.gif" title="Working... Please wait as this may take a moment..."></div>';
	...
	if ( $needle == '' ) {
		...
		echo '<br><a href="' . htmlentities( pb_backupbuddy::page_url() ) . '&parent_config=' . htmlentities( pb_backupbuddy::_GET( 'parent_config' ) ) . '" class="button secondary-button">&larr; ' .  __( 'back', 'it-l10n-backupbuddy' ) . '</a>';
		return;
	}
...
<div>
	<form action="<?php echo htmlentities( pb_backupbuddy::page_url() );?>&database_replace=1&parent_config=<?php echo htmlentities( pb_backupbuddy::_GET( 'parent_config' ) ); ?>" method="post">
