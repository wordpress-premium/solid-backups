...
use Solid_Backups\Strauss\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
...
class Transfer implements PromisorInterface
{
     ...
     *   callback accepts a single argument: Solid_Backups\Strauss\Aws\CommandInterface $command.
    ...
    private function createDownloadPromise()
    {
        ...
        return (new \Solid_Backups\Strauss\Aws\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \Solid_Backups\Strauss\Aws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return \Solid_Backups\Strauss\Aws\filter(
                \Solid_Backups\Strauss\Aws\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = \Solid_Backups\Strauss\Aws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return \Solid_Backups\Strauss\Aws\filter($files, function ($key) {
