...
use Solid_Backups\Strauss\GuzzleHttp\Exception\InvalidArgumentException;
    ...
    private function applyPathStyleEndpointCustomizations(
        ...
        if ($command->getName() == 'WriteGetObjectResponse') {
            $dnsSuffix = $this->endpointProvider
                ...
                ->getDnsSuffix();
            $fips = \Solid_Backups\Strauss\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \Solid_Backups\Strauss\Aws\strip_fips_pseudo_regions($this->region);
            $host =
