...
use Solid_Backups\Strauss\Aws\Arn\ArnParser;
use Solid_Backups\Strauss\Aws\Arn\ObjectLambdaAccessPointArn;
...
use Solid_Backups\Strauss\Aws\Arn\S3\OutpostsAccessPointArn;
use Solid_Backups\Strauss\Aws\Arn\S3\MultiRegionAccessPointArn;
use Solid_Backups\Strauss\Aws\Arn\S3\OutpostsArnInterface;
...
use Solid_Backups\Strauss\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(
        ...
        $region = \Solid_Backups\Strauss\Aws\strip_fips_pseudo_regions($region);
     ...
     *
     * @param $arn
     * @return \Solid_Backups\Strauss\Aws\Endpoint\Partition
     */
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!($clientPart->isRegionMatch($this->region, 's3'))) {
                $clientPart = $this->partitionProvider->getPartition(
                    \Solid_Backups\Strauss\Aws\strip_fips_pseudo_regions($this->region),
                    's3'
                );
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
