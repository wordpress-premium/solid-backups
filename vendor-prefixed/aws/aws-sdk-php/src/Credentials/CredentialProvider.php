<?php
namespace Solid_Backups\Strauss\Aws\Credentials;

use Solid_Backups\Strauss\Aws;
use Solid_Backups\Strauss\Aws\Api\DateTimeResult;
use Solid_Backups\Strauss\Aws\CacheInterface;
use Solid_Backups\Strauss\Aws\Exception\CredentialsException;
use Solid_Backups\Strauss\Aws\Sts\StsClient;
...
class CredentialProvider
{
     ...
     * @see Solid_Backups\Strauss\Aws\Credentials\InstanceProfileProvider for $config details.
     */
     ...
     * @see Solid_Backups\Strauss\Aws\Credentials\EcsCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see Solid_Backups\Strauss\Aws\Credentials\AssumeRoleCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see Solid_Backups\Strauss\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider for
     * $config details.
     */
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            $data = \Solid_Backups\Strauss\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: $filename");
            }
            if (!isset($data[$profile])) {
    ...
    {
        $profileData = \Solid_Backups\Strauss\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials'
            ...
            $configProfileData = \Solid_Backups\Strauss\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles() {
        ...
        if (file_exists($credFile)) {
            $profiles = \Solid_Backups\Strauss\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }

        if (file_exists($configFile)) {
            $configProfileData = \Solid_Backups\Strauss\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    private static function getSsoCredentials($profiles, $ssoProfileName, $filename, $config)
    {
        if (empty($config['ssoOidcClient'])) {
            ...
            $ssoOidcClient = new \Solid_Backups\Strauss\Aws\SSOOIDC\SSOOIDCClient([
        ...
        $tokenPromise = new \Solid_Backups\Strauss\Aws\Token\SsoTokenProvider(
            $ssoProfileName,
            $filename,
            $ssoOidcClient
        );
    ...
    private static function getCredentialsFromSsoService($ssoProfile, $clientRegion, $accessToken, $config)
    {
        if (empty($config['ssoClient'])) {
            $ssoClient = new \Solid_Backups\Strauss\Aws\SSO\SSOClient([
                'region' => $clientRegion,
                'version' => '2019-06-10',
                'credentials' => false
            ]);
