...
if ( false != $upgrade_options ) {
	...
	if ( '#PASSWORD#' == pb_backupbuddy::$options['import_password'] ) {
	...
	if ( ! empty( pb_backupbuddy::$options['ftp_server'] ) ) {
		...
		if ( 'ftp' == pb_backupbuddy::$options['ftp_type'] ) {
...
if ( pb_backupbuddy::$options['data_version'] < 6 ) {
	...
	foreach ( pb_backupbuddy::$options['schedules'] as &$schedule ) {
		if ( empty( $schedule['profile'] ) ) { // No profile set.
			if ( 'db' == $schedule['type'] ) {
				$schedule['profile'] = '1';
			}
			if ( 'full' == $schedule['type'] ) {
				$schedule['profile'] = '2';
			}
			unset( $schedule['type'] );
		}
		...
		if ( version_compare( PHP_VERSION, $php_minimum, '>=' ) ) { // Server's PHP is sufficient.
			...
			foreach ( pb_backupbuddy::$options['remote_destinations'] as $destination_id => $v1_dest ) {
				...
				if ( true === pb_backupbuddy_destination_stash2::test( $v2_dest ) ) {
					...
					pb_backupbuddy::$options['remote_destinations'][ $next_dest_key ] = $v1_dest; // New location for old stash1 settings.
					$next_dest_key++;
				}
